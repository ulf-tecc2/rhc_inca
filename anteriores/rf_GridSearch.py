...
ccp_alpha_range = np.linspace(0.03 , 0.05) 


param_grid_rf = {
    'ccp_alpha': [0.05] , # controlar a complexidade das árvores no conjunto florestal e prevenir overfitting. O parâmetro ccp_alpha (minimal cost-complexity pruning) controla a poda das árvores de decisão no modelo.
    'n_estimators': [80 ], # Número de árvores no ensemble. Mais árvores podem melhorar a acurácia, mas podem aumentar o custo computacional e levar ao overfitting.
                                              # Experimente valores entre 100 e 1000
    'max_features': [5],  # Profundidade máxima das árvores. Valores maiores permitem que o modelo capture padrões complexos, mas aumentam o risco de overfitting.
                                         # Teste valores entre 3 e 10.
    'min_samples_split': [30], # Número mínimo de amostras necessário para dividir um nó. Valores maiores simplificam as árvores e ajudam a evitar overfitting.
                            # Sugestão: Tente valores entre 2 e 10.
}

# RF4: {'n_estimators': 150, 'min_samples_split': 20, 'max_features': 10, 'ccp_alpha': 0.0}
...
if not df_restante.empty:
    # df_restante = df_restante.drop(columns=['rf_pred_class'])
